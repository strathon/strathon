...
Strathon's initial database schema. Brings the database from empty to the
v0 state: projects, traces, spans, span_events, span_links, policies,
api_keys, sessions, project_settings, halt_state, and the supporting
indexes. Detailed comments live inline.

This migration is intentionally large because it represents the entire
pre-Alembic baseline that existed as 001_initial.sql before migrations
were brought under Alembic management. Splitting it into smaller logical
migrations would only invent history we don't have.
...
# Tables to drop in downgrade, in reverse dependency order. Maintained
# manually because op.execute doesn't give Alembic a way to introspect what
# the SQL did. The list mirrors every CREATE TABLE in _UPGRADE_SQL, dropped
# in an order that respects foreign-key cascades.
_DOWNGRADE_SQL = r"""
DROP TABLE IF EXISTS halt_state CASCADE;
...
DROP TABLE IF EXISTS span_links CASCADE;
DROP TABLE IF EXISTS span_events CASCADE;
